<template>
    <header>
        <div class="sticky top-0 h-[7vh] bg-[#FFDF88] flex justify-between items-center px-[2%]">
            <h1 class="forum lg:text-2xl text-lg hover:underline font-bold text-black" @click="async () => {await navigateTo('/')}">SITHS Book Review</h1>
            <div class="flex lg:w-[40%] w-[70%] justify-around">
                ...
                <button @click="async () => {await navigateTo('/classes')}"
                    ...
                    Classes </button>
                <h2 class="forum lg:text-2xl text-lg hover:underline text-black" @click="async () => {await navigateTo('/settings')}">
                    username
                </h2>
            </div>
        </div>
