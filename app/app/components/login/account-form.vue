...
function logIn() {
    ...
    if (!email.value.includes("@nycstudents.net") && props.userType === "Student") {
        errorMessage.value = "Invalid email address."
    } else emailValid.value = true
    errorMessage.value = validatePassword(password.value)[0]
    passwordValid.value = validatePassword(password.value)[1]


    if (passwordValid.value && emailValid.value) {
        userStore.loggedIn = true
        userStore.userType = props.userType
        navigateTo("/")
    }
}
