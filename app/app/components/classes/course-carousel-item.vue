<template>
  <div
    class="shadow-md h-[94%] w-[40vw]! rounded-2xl p-4 carousel-item flex flex-col mx-2 overflow-y-scroll"
    :class="[
      course.isArchived ? 'bg-sky-100' : 'bg-white',
      deleted ? 'hidden' : '',
    ]"
  >
    <h3 class="text-slate-800 forum text-2xl text-center font-black">
      {{ course.instructor.split("@")[0] }}'s {{ course.name }}, Period
      {{ course.period }}
    </h3>
    <p class="text-lg italic forum text-slate-600 text-center">
      your role: {{ useUserStore().userType }}
    </p>
    ...
    >
      <p class="forum text-2xl text-black font-bold m-2 text-center">
        You have an upcoming review that you must complete by
        <span class="text-red-900 text-4xl">{{ "UPCOMINGREVIEW HERE" }}</span
        >.
      </p>
    ...
    >
      <p class="forum text-2xl text-black font-bold m-2 text-center">
        You have no upcoming reviews! Chill out and read a book.
      </p>
      <button
        @click="
          async () => {
        ...
        "
        class="forum text-2xl bg-white shadow-sm hover:shadow-lg active:shadow-none px-[5%] text-black rounded-full w-full transition-all duration-300 ease-in-out hover:bg-sky-100 active:bg-sky-300 hover:-translate-y-0.5 active:translate-y-1 py-[5%]"
      >
        View Books
      </button>
      <div class="h-1 rounded-full bg-slate-800 my-2"></div>
    ...
    <div class="w-full flex flex-col justify-between">
      ...
      >
        ...
        >
          ...
          <input
            ...
            placeholder="MM"
            maxlength="2"
            class="bg-slate-100 rounded-lg text-center focus:outline-none shadow-xs focus:shadow-md transition-all duration-300 ease-in-out text-sm px-0.5 w-[10%] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          />
          /
          <input
            v-model="newDateD"
          ...
          <Add
            ...
            @click="(validateReviewDue(), makeAssignment())"
          />
        </button>
        <div
          v-if="newDateError"
        ...
        >
          Edit Class Name
          <input
            v-model="newClassNameInput"
            type="text"
            placeholder="New Name..."
            maxlength="35"
            class="bg-slate-100 rounded-lg text-center focus:outline-none shadow-xs focus:shadow-md transition-all duration-300 ease-in-out text-sm px-0.5 w-[50%]"
          />
          <div
      ...
      <h2
        v-if="course.isArchived"
        class="forum text-black italic text-center text-lg mt-1 -mb-1"
      >
    ...
    <transition name="highlight-background">
      <div
        class="w-screen h-screen bg-taupe-700/40 fixed top-0 left-0"
        v-if="viewingStudents || showClassCode"
      ></div>
    </transition>
    <transition name="students-modal">
      <ClassesStudentsModal
        v-if="viewingStudents"
        @close="viewingStudents = false"
      ></ClassesStudentsModal>
    </transition>
    <transition name="class-code-modal">
      <div
        class="fixed left-[10%] top-[10%] h-[80vh] w-[80vw] bg-linear-to-tr from-sky-100 to-slate-100 rounded-3xl shadow-lg p-[2%] gap-4 justify-between flex-col flex py-[8%] md:py-[4%]"
        v-if="showClassCode"
      >
        <div class="flex justify-between w-full items-center flex-col">
          <div
            class="flex flex-row justify-between w-full h-[10%] items-center"
          >
            <X
              :size="48"
              color="black"
              @click="showClassCode = false"
              class="rounded-full hover:bg-sky-400/20 p-1 transition-colors duration-200 ease-in"
            />
          </div>
        </div>
        <h3 class="text-[3rem] font-bold forum text-black text-center mt-10">
          CLASS CODE:
        </h3>
        <h3
          class="text-[7rem] font-black forum text-black text-center mb-[30%]"
        >
          {{ course.classcode }}
        </h3>
      </div>
...
watch(
  () => [newDateD.value, newDateY.value, newDateM.value],
  () => {
...
function fulfillsLeapYear() {
  if (newDateM.value === 2 && newDateD.value === 29 && newDateY.value) {
    if (
      newDateY.value % 4 === 0 &&
      (newDateY.value % 100 !== 0 || newDateY.value % 400 === 0)
    ) {
      return true;
    }
  }
...
async function makeAssignment() {
  try {
    const response = await fetch(`${config.public.apiBase}/api/assignments/`, {
      ...
      body: JSON.stringify({
        course: prop.course.id,
        dueDate: `${newDateY.value}-${newDateM.value}-${newDateD.value}`,
      }),
    });
    return response;
  } catch (error) {
...
async function archiveClass() {
  try {
    const response = await fetch(
      `${config.public.apiBase}/api/courses/${prop.course.id}/`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${userStore.accessToken}`,
