<template>
    <div class="fixed left-[10%] top-[10%] h-[80vh] w-[80vw] bg-slate-100 rounded-3xl shadow-lg p-[2%] gap-4 justify-between flex-col flex md:py-[4%]">
        ...
        <input v-model="courseName" id="classCodeInput" placeholder="English, AP Language, etc." type="text" maxlength="35" class="bg-white h-[20%] text-[2.7rem] rounded-2xl text-center shadow-md focus:shadow-lg forum transition duration-300 ease-in-out placeholder-slate-400 text-black focus:outline-none px-1">
        <div class="flex flex-row flex-wrap h-[20%] items-center gap-y-0">
           ...
           <div v-for="i in 9" class="forum text-black text-2xl flex gap-2 items-center flex-row p-[1%]">
                       <input id="periodsInput" type="checkbox" class="checkbox border-2 border-slate-800 p-[2%] bg-slate-700 shadow-md"  @click="periodsInput[i-1] = !periodsInput[i-1]"/>
               {{ i }}
           </div>
        </div>
        <button 
