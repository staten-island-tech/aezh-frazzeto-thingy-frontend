<template>
  <div class="relative w-[18%] h-[45%] shrink-0">
    ...
    <transition name="slide-fade">
        <aside class="absolute bg-white h-full w-[150%] translate-x-[18vw] rounded-lg shadow-lg p-2 shadow-slate-600 z-1 flex flex-col justify-between py-4" v-if="isSelected">
            <div>
                <h3 class="forum text-center text-md"> {{ book.title ?? 'Unnamed Book' }} <span class="text-xs italic"> (written by {{ book.author.name ?? 'unknown' }}) </span></h3>
