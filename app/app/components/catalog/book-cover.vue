    ...
    @click="async () => { await navigateTo('/book/' + book.id) }" :key="book.id" :book="book">
      ...
      <div
        ...
        <div class="flex flex-col items-center justify-center w-[20vw] h-[20vw]">
          <h2 class="forum text-[1.25rem] text-black min-h-[40%] text-center">{{ book.name }}</h2>
          <h3 class="forum text-[1rem] h-[30%] text-black text-center">{{ book.author }}</h3>
          <h3 class="forum text-[1rem] h-[30%] text-black text-center">{{ book.rating }}/5</h3>
        </div>
      </div>
      </transition>
    </button>
