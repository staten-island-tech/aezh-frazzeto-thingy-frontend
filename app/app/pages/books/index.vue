<template>
  ...
  >
  <catalog-cat-header @return-search="async (booksReturn, count) => {
    setBooks(booksReturn, count)
  }"> </catalog-cat-header>
    <div class="flex gap-4 justify-between p-[1%] forum text-black font-bold rounded-2xl shadow-lg text-lg bg-slate-200 w-full h-[70vh]! mt-[3vh] overflow-y-scroll flex-wrap relative">
      <catalog-book-cover v-if="showCatalog"
        v-for="book in (books as any).results"
        :key="book.title"
        :book="book"
      > </catalog-book-cover>
      <div class="bg-white fixed lg:w-[40%] w-[60%] h-[10%] lg:left-[30%] left-[20%] rounded-2xl bottom-2 shadow-md flex justify-between items-center px-2">
